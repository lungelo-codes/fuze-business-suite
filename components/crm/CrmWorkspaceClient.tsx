...
function SalesDocModal({ record, kind, onClose, onSaved }: { record: DrawerRecord; kind: "quote" | "invoice" | "sales-order" | "contract"; onClose: () => void; onSaved: (message: string) => void }) {
  const [lines, setLines] = useState([{ item_code: "", item_name: "", qty: "1", rate: "0", description: "" }]);
  ...
  useEffect(() => { let active = true; apiFetch(`/api/crm/sales-options?doctype=${encodeURIComponent(docType)}&limit=100`).then((json) => { const data = unwrap<any>(json); if (!active) return; setItems(data.items || []); setTemplates(data.templates || {}); }).catch(() => undefined); return () => { active = false; }; }, [docType]);
  function updateLine(i: number, key: string, value: string) { setLines((rows) => rows.map((row, idx) => idx === i ? { ...row, [key]: value } : row)); }
  function pickItem(i: number, code: string) { const item = items.find((x) => String(x.item_code || x.name) === code); setLines((rows) => rows.map((row, idx) => idx === i ? { ...row, item_code: code, item_name: item?.item_name || item?.name || row.item_name, description: item?.description || row.description, rate: String(item?.standard_rate ?? row.rate ?? 0) } : row)); }
  function addLine() { setLines((rows) => [...rows, { item_code: "", item_name: "", qty: "1", rate: "0", description: "" }]); }
  ...
  async function submit() {
    ...
    try {
      ...
      const payload = kind === "contract"
        ? { contract_name: contractName || `Contract - ${record.title}`, terms_template: termsTemplate, print_format: printFormat, email_template: emailTemplate }
        : { items: lines.map((row) => ({ item_code: row.item_code || undefined, item_name: row.item_name || "Service", description: row.description, qty: Number(row.qty || 1), rate: Number(row.rate || 0) })), taxes_and_charges: taxTemplate || undefined, tax_template: taxTemplate || undefined, tc_name: termsTemplate || undefined, terms_template: termsTemplate || undefined, print_format: printFormat || undefined, email_template: emailTemplate || undefined };
      ...
      const label = kind === "quote" ? "Quote" : kind === "invoice" ? "Invoice" : kind === "sales-order" ? "Sales Order" : "Contract";
      onSaved(`${label} created: ${id}`);
    } catch (e) { setError(e instanceof Error ? e.message : `Could not create ${kind}`); setSaving(false); }
  }
  const title = kind === "quote" ? "Create Quote" : kind === "invoice" ? "Create Invoice" : kind === "sales-order" ? "Create Sales Order" : "Create Contract";
  return <Modal title={title} subtitle="Create and send sales documents from this customer record." onClose={onClose} width={920}><ErrorBanner message={error} />
    {kind === "contract" ? <div className="crm-form-grid"><Field label="Contract name"><input value={contractName} onChange={(e) => setContractName(e.target.value)} placeholder={`Contract - ${record.title}`} /></Field></div> : <div className="crm-line-items"><div className="crm-line-head"><b>Items / services</b><button className="btn btn-small" onClick={addLine}>Add line</button></div>{lines.map((line, i) => <div className="crm-line-row" key={i}><select value={line.item_code} onChange={(e) => pickItem(i, e.target.value)}><option value="">New service / manual item</option>{items.map((item) => <option key={item.item_code || item.name} value={item.item_code || item.name}>{item.item_name || item.name}</option>)}</select><input placeholder="Item / service name" value={line.item_name} onChange={(e) => updateLine(i, "item_name", e.target.value)} /><input type="number" placeholder="Qty" value={line.qty} onChange={(e) => updateLine(i, "qty", e.target.value)} /><input type="number" placeholder="Rate" value={line.rate} onChange={(e) => updateLine(i, "rate", e.target.value)} /><button className="btn btn-small" onClick={() => removeLine(i)}>Remove</button></div>)}<div className="crm-doc-total">Total {money(total)}</div></div>}
    <div className="crm-form-grid"><Field label="Terms template"><select value={termsTemplate} onChange={(e) => setTermsTemplate(e.target.value)}><option value="">Default terms</option>{(templates.terms || []).map((t: any) => <option key={t.name} value={t.name}>{t.title || t.name}</option>)}</select></Field>{kind !== "contract" && <Field label="Tax template"><select value={taxTemplate} onChange={(e) => setTaxTemplate(e.target.value)}><option value="">No tax template</option>{(templates.taxes || []).map((t: any) => <option key={t.name} value={t.name}>{t.title || t.name}</option>)}</select></Field>}<Field label="Print template"><select value={printFormat} onChange={(e) => setPrintFormat(e.target.value)}><option value="">Default print format</option>{(templates.print_formats || []).map((t: any) => <option key={t.name} value={t.name}>{t.name}</option>)}</select></Field><Field label="Email template"><select value={emailTemplate} onChange={(e) => setEmailTemplate(e.target.value)}><option value="">No email template</option>{(templates.email_templates || []).map((t: any) => <option key={t.name} value={t.name}>{t.subject || t.name}</option>)}</select></Field></div>
    <div className="crm-modal-actions"><button className="btn" onClick={onClose}>Cancel</button><button className="btn btn-primary" disabled={saving} onClick={submit}>{saving ? "Creating…" : title}</button></div></Modal>;
}

function DetailDrawer({ record, crmMode, onClose, onRefresh }: { record: DrawerRecord; crmMode: CrmMode; onClose: () => void; onRefresh: () => void }) {
  const [active, setActive] = useState<"life" | "timeline" | "emails" | "calls" | "notes" | "tasks" | "comments" | "attachments">("life");
  const [detail, setDetail] = useState<any>(null); const [life, setLife] = useState<Customer360 | null>(null); const [loading, setLoading] = useState(true); const [message, setMessage] = useState(""); const [modal, setModal] = useState<"quote" | "invoice" | "sales-order" | "contract" | null>(null);
  ...
  async function attachFile() { if (!fileUrl.trim()) return; try { await apiFetch("/api/crm/attachments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reference_doctype: doctype, reference_name: record.id, file_url: fileUrl }) }); setFileUrl(""); setMessage("Attachment linked"); await load(); } catch (e) { setMessage(e instanceof Error ? e.message : "Could not attach file"); } }
