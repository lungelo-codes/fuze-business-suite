...
const TABS: { id: HrTab; label: string; subtitle: string }[] = [
  ...
  { id: "setup", label: "HR Setup", subtitle: "Departments, branches, designations, grades, employment types, leave and shift masters" },
...
const SOURCES: Record<HrTab, Source[]> = {
  ...
  setup: [
    ...
    { key: "branches", label: "Branches", doctype: "Branch", create: "New Branch", columns: [{ key: "name", label: "Branch" }], fields: [{ name: "branch", label: "Branch Name", required: true }] },
...
function FieldInput({ field, value, onChange, options, onOptionCreated }: { field: Field; value: any; onChange: (v: any) => void; options: Record<string, { name: string; label: string }[]>; onOptionCreated: (doctype: string, row: { name: string; label: string }) => void }) {
  ...
  async function addOption() {
    ...
    const row = (json.data || json.record || json.message || { name: label.trim(), label: label.trim() }) as { name: string; label: string };
    onOptionCreated(field.link, row);
    onChange(row.name || label.trim());
  }
  if (field.options || field.link) return <div className="hr-link-field"><select value={String(value ?? "")} onChange={(e) => e.target.value === "__add_new__" ? void addOption() : onChange(e.target.value)}><option value="">Select…</option>{field.options?.map((o) => <option key={o} value={o}>{o}</option>)}{opts.map((o) => <option key={o.name} value={o.name}>{o.label || o.name}</option>)}{field.link && <option value="__add_new__">+ Add new {field.link}</option>}</select>{field.link && <button type="button" className="btn btn-small" onClick={addOption}>Add</button>}</div>;
...
function Modal({ mode, source, record, options, onClose, onSaved, onOptionCreated }: { mode: "create" | "edit"; source: Source; record?: Any; options: Record<string, { name: string; label: string }[]>; onClose: () => void; onSaved: () => void; onOptionCreated: (doctype: string, row: { name: string; label: string }) => void }) {
...
export default function HRWorkspaceClient({ initialTab }: Props) {
  ...
  function addOptionToState(doctype: string, row: { name: string; label: string }) {
