...
export default function CrudModulePage({ moduleId, config, initialRows = [] }: { moduleId: string; config: CrudModuleConfig; initialRows?: Row[] }) {
  ...
  return <div className="space-y-6">
    ...
    {stageCards.length > 0 ? <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">{stageCards.map((card) => <button key={card.stage} type="button" onClick={() => setStageFilter(stageFilter === card.stage ? '' : card.stage)} className={`rounded-2xl border p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${stageFilter === card.stage ? 'border-emerald-300 bg-emerald-50' : 'border-slate-200 bg-white'}`}><div className="text-[11px] font-black uppercase tracking-wide text-slate-500">{statusField.replace(/_/g, ' ')}</div><div className="mt-2 flex items-center justify-between gap-3"><StatusChip status={card.stage} /><span className="text-2xl font-black text-[#15132E]">{card.count}</span></div><div className="mt-2 text-xs font-semibold text-slate-500">Click to list this stage</div></button>)}</div> : null}
