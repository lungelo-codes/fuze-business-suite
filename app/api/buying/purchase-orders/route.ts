...
export async function GET(req: Request) {
  ...
  try { return NextResponse.json(await erpMethod("procurement.get_purchase_orders", args)); }
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json(await erpMethod("procurement.create_purchase_order", { data: body }), { status: 201 });
  } catch (e: any) { return NextResponse.json({ error: e?.message }, { status: 500 }); }
}
