...
function cleanCompanyFromSite(site?: string) {
...
async function fetchUserRoles(backendUrl: string, email: string, sid?: string): Promise<string[]> {
  ...
  try {
    const res = await fetch(
      `${backendUrl}/api/resource/Has%20Role?filters=${filters}&fields=${fields}`,
...
async function fetchTenantContext(site?: string, email?: string): Promise<TenantContext> {
  ...
  if (!MASTER_ERPNEXT_URL || !ERPNEXT_API_KEY || !ERPNEXT_API_SECRET) {
    return {
      companyName: cleanCompanyFromSite(site),
      plan: fallbackPlan,
  ...
  try {
    ...
    const tenantJson = (await tenantRes.json()) as { data?: Record<string, unknown>[] };
    const tenant = tenantJson.data?.[0];
    ...
    const plan =
      String(tenant.plan || tenant.subscription_plan || "").trim() ||
      fallbackPlan;
    ...
    if (!modules.length && tenantName) {
      const childFilters = encodeURIComponent(
        JSON.stringify([
          ["parent", "=", tenantName],
        ])
      );

      const childFields = encodeURIComponent(
        JSON.stringify(["module", "module_key", "saas_module", "enabled"])
      );
      ...
      if (moduleRes.ok) {
        const moduleJson = (await moduleRes.json()) as { data?: Record<string, unknown>[] };
        modules = (moduleJson.data || [])
    ...
    return {
      companyName: String(tenant.company_name || cleanCompanyFromSite(site)),
      plan,
      modules,
    };
  } catch {
...
export async function POST(req: Request) {
  try {
    ...
    response.cookies.set("tenant_site", cleanSite, { ...common, maxAge: 365 * 86400 });
    response.cookies.set("tenant_backend", backendUrl, { ...common, maxAge: 365 * 86400 });
    response.cookies.set(TENANT_COOKIE, cleanSite, { ...common, maxAge: 365 * 86400 });

    response.cookies.set(ROLE_COOKIE, isAdmin ? "admin" : "customer", common);
    response.cookies.set(PLAN_COOKIE, tenantContext.plan, common);
    response.cookies.set(COMPANY_COOKIE, encodeURIComponent(tenantContext.companyName), common);
    response.cookies.set(
      MODULE_COOKIE,
      encodeURIComponent(JSON.stringify(tenantContext.modules)),
      common
    );
