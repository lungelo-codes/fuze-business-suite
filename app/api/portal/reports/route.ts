...
export async function GET() {
  try {
    const [
      invoices, payments, tasks, issues,
      employees, leaveList,
      ...
      safe(() => erpList<Row>("Sales Order",      { fields:["name","transaction_date","grand_total","status","customer"], limit:300 }), []),
      ...
      // Salary Slips for payroll chart — direct erpList (numeric gross_pay / net_pay fields)
      safe(() => erpList<Row>("Salary Slip",      { fields:["name","employee_name","start_date","end_date","gross_pay","net_pay","docstatus"], limit:200, orderBy:"start_date asc" }), []),
    ]);
    ...
    const invoicesByMonth = Object.entries(invMap).slice(-12).map(([month, v]) => ({ month, ...v }));

    const payMap: Record<string, number> = {};
    for (const p of payments) {
      const k = monthKey(p.posting_date);
      if (k !== "Unknown") payMap[k] = (payMap[k] || 0) + Number(p.paid_amount || 0);
    }
    const paymentsByMonth = Object.entries(payMap).slice(-12).map(([month, total]) => ({ month, total }));
    ...
    leaveList.forEach((l) => { const t = String(l.leave_type || "Other"); leaveTypeMap[t] = (leaveTypeMap[t] || 0) + 1; });
    const leaveByType = Object.entries(leaveTypeMap).map(([type, count]) => ({ type, count }));

    // Payroll by month — grouped from salary slips using numeric gross_pay / net_pay
    const payrollMap: Record<string, { gross:number; net:number }> = {};
    salarySlips.forEach((slip) => {
      const k = monthKey(slip.start_date);
      if (k === "Unknown") return;
      if (!payrollMap[k]) payrollMap[k] = { gross:0, net:0 };
      payrollMap[k].gross += Number(slip.gross_pay || 0);
      payrollMap[k].net   += Number(slip.net_pay   || 0);
    });
    const payrollByMonth = Object.entries(payrollMap)
      .slice(-12)
      .map(([month, v]) => ({ month, ...v }))
