...
export async function GET(req: Request) {
  ...
  try { return NextResponse.json(await erpMethod("fuze_suite.api.selling.get_sales_orders", args)); }
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json(await erpMethod("fuze_suite.api.selling.create_sales_order", { data: body }), { status: 201 });
  } catch (e: any) { return NextResponse.json({ error: e?.message }, { status: 500 }); }
}
