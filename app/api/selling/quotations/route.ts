...
export async function GET(req: Request) {
  ...
  try { return NextResponse.json(await erpMethod("selling.get_quotations", args)); }
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json(await erpMethod("selling.create_quotation", { data: body }), { status: 201 });
  } catch (e: any) { return NextResponse.json({ error: e?.message }, { status: 500 }); }
}
