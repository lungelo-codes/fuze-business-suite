...
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    const result = await erpMethod("fuze_suite.api.crm.create_contract_from_crm", { deal: params.id, data: body });
  ...
  try {
    const row = await createCrmSalesDocument("deal", params.id, "contract", body);
