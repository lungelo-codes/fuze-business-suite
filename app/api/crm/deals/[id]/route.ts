...
async function safeList(doctype: string, filters: unknown[], fields: string[]): Promise<Row[]> { try { return await erpList<Row>(doctype, { fields, filters, limit: 80, orderBy: "modified desc" }); } catch { return []; } }
...
export async function GET(_req: Request, { params }: Params) {
  try {
    let result: any = null;
    try { result = await erpMethod("crm.get_deal", { deal: params.id }); } catch { result = null; }
    const base = docFrom(result);
    const deal = base.deal || base.opportunity || base.doc || base;
    const fallbackDeal = Object.keys(deal || {}).length ? deal : (await safeList("Opportunity", [["name", "=", params.id]], ["name", "title", "party_name", "customer_name", "status", "sales_stage", "opportunity_amount", "modified"]))[0] || {};
    ...
    return NextResponse.json({ data: { ...base, deal: fallbackDeal, communications, tasks, activity } });
  } catch (error: any) {
    return NextResponse.json({ error: error?.message || "Opportunity not found" }, { status: error?.status || 404 });
  }
}
...
export async function PUT(req: Request, { params }: Params) {
  try {
    ...
    if (body._action === "lost") {
      const result = await erpMethod("crm.mark_deal_lost", { deal: params.id, lost_reason: body.lost_reason ?? null, notes: body.notes ?? null });
      return NextResponse.json(result);
    }
    const result = await erpMethod("crm.update_deal", { deal: params.id, data: body });
    return NextResponse.json(result);
  } catch (error: any) {
    return NextResponse.json({ error: error?.message || "Failed to update opportunity" }, { status: error?.status || 500 });
  }
}
