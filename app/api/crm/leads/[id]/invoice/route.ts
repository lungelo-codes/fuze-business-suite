...
export async function POST(req: Request, { params }: Params) {
  ...
  try {
    const result = await erpMethod("fuze_suite.api.crm.create_invoice_from_crm", { lead: params.id, data: body });
  ...
  try {
    const row = await createCrmSalesDocument("lead", params.id, "invoice", body);
