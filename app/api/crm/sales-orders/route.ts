...
async function fallback(limit: number): Promise<Row[]> { return erpList<Row>("Sales Order", { fields: ["name","customer","customer_name","transaction_date","delivery_date","status","docstatus","grand_total","rounded_total","currency","modified"], limit, orderBy: "modified desc" }); }
...
export async function GET(req: Request) {
  ...
  try {
    let rows: Row[] = [];
    try { rows = rowsFrom(await erpMethod("sales.get_sales_orders", args)); } catch { rows = []; }
  ...
  } catch { return NextResponse.json({ success: true, data: [], sales_orders: [], orders: [], count: 0 }); }
}
