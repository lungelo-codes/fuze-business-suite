...
export async function GET(req: Request) {
  ...
  try {
    const result = await erpMethod("fuze_suite.api.crm.get_custom_fields", { doctype });
...
export async function POST(req: Request) {
  try {
    ...
    const result = await erpMethod("fuze_suite.api.crm.create_custom_field", { doctype, data });
