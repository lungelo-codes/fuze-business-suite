import { erpCreate, erpGet, erpList, erpMethod, erpPatch, erpPost } from "@/lib/server/erpnext";
...
export async function approvePaymentProof(name: string, input: Row) { try { const r = await erpMethod<Row>("fuze_suite.api.saas.approve_payment_proof", { name, ...input }); if (r) return r; } catch {} const proof = await erpPatch<Row>("Payment Proof", name, { status: "Approved" }); const invoice = String(input.invoice || input.sales_invoice || ""); if (invoice) { try { const inv = await erpGet<{ data?: Row }>(`/api/resource/Sales%20Invoice/${encodeURIComponent(invoice)}`); const doc = inv.data || {}; const amount = n(input.amount || doc.outstanding_amount || doc.grand_total); await erpPost("/api/resource/Payment Entry", { payment_type: "Receive", party_type: "Customer", party: doc.customer, paid_amount: amount, received_amount: amount, reference_no: input.reference_number || name, reference_date: today(), references: [{ reference_doctype: "Sales Invoice", reference_name: invoice, allocated_amount: amount }] }); } catch {} } return proof; }
