...
const DEFAULT_ERPNEXT_URL =
  process.env.ERPNEXT_URL || process.env.NEXT_PUBLIC_ERPNEXT_URL;
...
function getRuntimeBackendUrl(): string {
  try {
    const tenantBackend = cookies().get("tenant_backend")?.value;
    if (tenantBackend) return tenantBackend.replace(/\/$/, "");
  } catch {}

  if (!DEFAULT_ERPNEXT_URL) throw new BusinessSuiteError("Missing ERPNEXT_URL");
  return DEFAULT_ERPNEXT_URL.replace(/\/$/, "");
}
...
export async function erpPatch<T>(
  doctype: string,
  name: string,
  body: Record<string, unknown>
): Promise<T> {
  ...
  const response = await parseResponse<ERPDocumentResponse<T>>(
    res,
...
export async function erpCreate<T>(
  doctype: string,
...
export async function erpMethod<T>(
  method: string,
  body: Record<string, unknown>
): Promise<T | null> {
  const response = await erpPost<ERPDocumentResponse<T>>(
