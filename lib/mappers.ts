...
export function mapCompliance(data: unknown[], kind: ComplianceKind): ComplianceRecord[] {
  return data.map((item): ComplianceRecord => {
    ...
    return {
      ...
      company: s(r.company),
      status: s(r.status),
      due_date: s(r.due_date || r.to_date || r.submission_date),
      submission_date: s(r.submission_date),
      return_year: n(r.return_year || r.year || r.declaration_year),
      period: s(r.period || (r.month && r.year ? `${r.month}/${r.year}` : r.declaration_month && r.declaration_year ? `${r.declaration_month}/${r.declaration_year}` : "")),
      amount_due: n(r.amount_due || r.total_emp201 || r.total_uif || r.total_sdl || r.vat_payable || r.cipc_fee),
      total_due: n(r.total_due || r.net_vat || r.total_emp201 || r.total_uif || r.total_sdl || r.cipc_fee),
      kind,
      modified: s(r.modified),
      owner: s(r.owner)
    };
